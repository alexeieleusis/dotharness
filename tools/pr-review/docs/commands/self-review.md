...
### opencode `--standalone` project misrouting (ongoing)

Observed in production (PR #9670, #9676, and others): an opencode `--standalone` backend process, `Popen`'d with the correct `cwd`, was later found (via `lsof -p <pid> | grep cwd`) operating out of a *different* directory — concretely, wherever the harness's own long-running loop script (`runPrReviewOpencode.sh`) happened to `cd` into before invoking it. Confirmed intermittent (the same PR got a correct reply for one comment and a wrong-directory failure for another, minutes apart) and not explained by any harness-side bug found so far — `subprocess.Popen`'s `cwd` argument was verified correct in every case; the divergence happens inside opencode itself. Root cause not yet confirmed; mitigations below reduce exposure without claiming to fix the underlying opencode behavior:

- **`--auto`:** opencode v2's actual replacement for the removed `--dangerously-skip-permissions` ("auto-approve permissions that are not explicitly denied"). The original v2 migration assumed non-interactive `run` auto-approves without it; that's true for edit/bash, but the global `opencode.json` defaults `external_directory` permission to `"ask"` for anything not explicitly allowlisted, and it's untested whether a headless `run` hitting that prompt without `--auto` fails safe or falls back to some other project. Added as a plausible contributor, not a confirmed fix.
- **Global opencode serialization:** every opencode backend invocation, across every config/tool on this host, is now serialized behind one `flock` (`_OPENCODE_GLOBAL_LOCK_KEY` in `backend.py`), not just within one runner's batch. Directly closes any concurrency-dependent version of this bug, at the cost of throughput (opencode-backed runs across all tools now queue behind each other host-wide).
- **`_CwdDivergenceMonitor` (hard safety net):** while a backend process runs, a background thread polls its actual OS-level cwd via `lsof` every few seconds. If it no longer matches the directory it was launched in, the process is killed immediately and `Backend.run()` raises `BackendCwdDivergedError` instead of returning — so a misrouted run can never produce a trusted comment or commit, whether or not the above two mitigations actually address the root cause. Each runner already treats a `backend.run()` exception as "skip this comment, log it, move on," so this fails closed rather than posting bad output.
