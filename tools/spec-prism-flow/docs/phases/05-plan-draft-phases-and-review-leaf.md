...
**Open decision (flagged, not yet resolved — implement at heading granularity below until resolved otherwise):** "for each numbered `requirements.md` section" (item 3 above) never defines the granularity of "section" — every `##`/`###` heading (top-level §0–§12, plus subsections like §7.1–§7.3), or every numbered item nested inside a subsection's prose (e.g. §7.2's elicitation steps 1–8, §11's decision-log entries)? The two grains give very different coverage semantics: at heading grain, §7 counts as "covered" once any phase quotes anything from §7.1–§7.3, hiding gaps at the step level; at item grain, coverage is per-step and much stricter. This implementation treats every markdown heading (`##` or `###`) as a section — matching the `"§7.2"` match example already given — and does not separately track numbered items within a subsection's body as their own sections. Flagged here for a human to confirm or override before implementation.
