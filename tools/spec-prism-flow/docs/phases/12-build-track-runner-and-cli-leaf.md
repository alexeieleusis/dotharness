...
- `spec-prism-flow build status` — reads `{config.plan.phase_dir}/*-leaf.md` (never a hardcoded `docs/phases`, per Phase 01's required, no-default `plan.phase_dir`) + the completion log, prints one row per phase: number, name, status (`pending`/`in progress`/`escalated`/`merged`), address-comments cycle count, escalation count. Plain-text table (`click.echo` with fixed-width formatting) — no `rich.Table` dependency.
