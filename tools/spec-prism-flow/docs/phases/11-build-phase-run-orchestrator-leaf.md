...
`PhaseRunResult` frozen dataclass: `phase_number: int`, `merged: bool`, `completion_record: CompletionRecord`. `run_phase(clone: Path, config, phase: PhaseFile, *, toolchain: Toolchain | None = None, dry_run: bool = False, auto_merge: bool = True, resume: bool = False, strict: bool = False) -> PhaseRunResult`, executing, in order:
...
2. **Push + PR** — `push_branch`, `fetch_resync`, `pr_create` (Phase 08), then persist the returned PR number/URL via Phase 08's resume-state save, with `cycle_index=0`. (When resuming with an existing PR: skip steps 1–2 entirely, reuse the existing PR number/URL and the resume state's `cycle_index`, proceed directly to step 3.)
3–6. **Iterate loop** — one shared `RetryBudget` (Phase 06) with `cycle_index` incrementing each pass, seeded from the resume state's `cycle_index` on a resumed run (0 otherwise) so a crash-and-resume sequence keeps counting against the same budget instead of restarting it; `RetryBudgetExhausted` raised once `cycle_index` exceeds `config.build.max_retry_cycles` *before* running that cycle's work. Each cycle: `fetch_resync` + `diff_paths` + `scope_check` (re-check, since address-comments below may modify files outside the original diff), static-analysis scan+conditional-post (Phase 09, only if `config.vibe_heal.enabled`), self-review+address-comments (Phase 09, only if `config.review.enabled`) followed by `commit_all` + `push_branch` if address-comments modified the clone (so the next cycle's `fetch_resync` doesn't discard those fixes — this is the loop's second-and-later `commit_all` call, per 7.1's dry-run stub), then re-save the resume state (Phase 08) with the cycle's updated `cycle_index` so a crash after this point resumes the budget count correctly, `unresolved_thread_count` (Phase 08) — if `> 0`, loop again; else run `manual_test_prompt` (Phase 10, passing `--strict`'s effect through as a parameter), which returns one of three `ManualTestOutcome`s: `passed=True` proceeds to step 7; `passed=False, retry=True` loops again exactly like the `unresolved_thread_count > 0` case above — back to the top of the cycle (`fetch_resync` + `diff_paths` + `scope_check`, then static-analysis and review again), consuming one more `cycle_index` pass against the shared `RetryBudget` regardless of `--strict`; `passed=False, retry=False` is the terminal failure — under `--strict` it raises Phase 06's `ManualTestFailed` instead of looping, non-strict records it and proceeds to step 7 without blocking.
7. **Merge** — `diff_stat` against the base ref (for Phase 10's completion record), run `merge_gates_run` (Phase 06); if `auto_merge` is `False`, return without merging — `PhaseRunResult(phase_number=phase.number, merged=False, completion_record=...)`, where `completion_record` is a full `CompletionRecord` built from data already in hand at this point (`phase.number`/`phase.name`, the PR number/URL/opened-at persisted in step 2, `manual_test_first_try_pass` and the cycle count from the iterate loop, this step's `diff_stat` for the `pr_diff_*` fields), with `pr_merged_at=None` and `escalation_reason=None` since this path hasn't merged or escalated.
