...
3–6. **Iterate loop** — one shared `RetryBudget` (Phase 06) with `cycle_index` incrementing each pass; `RetryBudgetExhausted` raised once `cycle_index` exceeds `config.build.max_retry_cycles` *before* running that cycle's work. Each cycle: `fetch_resync` + `diff_paths` + `scope_check` (re-check, since address-comments below may modify files outside the original diff), static-analysis scan+conditional-post (Phase 09, only if `config.vibe_heal.enabled`), self-review+address-comments (Phase 09, only if `config.review.enabled`) followed by `commit_all` + `push_branch` if address-comments modified the clone (so the next cycle's `fetch_resync` doesn't discard those fixes — this is the loop's second-and-later `commit_all` call, per 7.1's dry-run stub), `unresolved_thread_count` (Phase 08) — if `> 0`, loop again; else run `manual_test_prompt` (Phase 10, passing `--strict`'s effect through as a parameter), which returns one of three `ManualTestOutcome`s: `passed=True` proceeds to step 7; `passed=False, retry=True` loops again exactly like the `unresolved_thread_count > 0` case above — back to the top of the cycle (`fetch_resync` + `diff_paths` + `scope_check`, then static-analysis and review again), consuming one more `cycle_index` pass against the shared `RetryBudget` regardless of `--strict`; `passed=False, retry=False` is the terminal failure — under `--strict` it raises Phase 06's `ManualTestFailed` instead of looping, non-strict records it and proceeds to step 7 without blocking.
