...
## Acceptance criteria
- `run_self_review`/`run_address_comments` invoke `uv run --project <harness_tool_dir> harness run --config <harness_config> <subcommand>` with `cwd=clone`, and raise `HarnessCommandError` (carrying the full args/returncode/stderr) on non-zero exit.
...
- With `config.review.enabled = False`, call `run_self_review` in a scratch script and confirm (e.g. via a monkeypatched `subprocess.run` that raises if called) that no subprocess is spawned.
