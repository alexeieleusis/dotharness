...
- **Plugin isolation for opencode (`assert_no_opencode_plugins`):** `--standalone` above says nothing about a globally-installed plugin acting independently of that per-invocation isolation — a gap opencode v1's removed `--pure` flag used to close. Before every opencode invocation, `backend.py` now runs `opencode plugin list` and aborts the whole run if it reports anything other than "No plugins found", rather than relying on an unverified, point-in-time check of one host's config. **Caveat:** this is checked once per invocation, not continuously — a plugin installed mid-run, after the check but before the backend process exits, would still slip through.
