...
- `BuildConfig` includes `max_retry_cycles: int` (default 3) and `state_dir: Path` (default `~/.local/share/dotharness/spec-prism-flow/state`); `ReviewConfig`/`VibeHealConfig` include a tool-dir path field each; a `[harness]` section includes `knowledge_dir` (default `~/.harness/knowledge`).
