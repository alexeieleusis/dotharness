...
- `spec-prism-flow build run [--start N] [--stop N] [--dry-run] [--resume]` — reads `config.build.workers`; `workers == 1` calls `run_track` (sequential), `workers > 1` calls `run_parallel` (parallel mode; concurrent completion-log writes across its leaves are conflict-safe per §7.2's `append_and_commit` retry loop — no extra coordination needed at the CLI layer). `--start`/`--stop` apply to sequential mode only (passing both together in parallel mode is a `click.UsageError`). On an `OrchestrationError`, print the escalation (message + `next_command` hint if present) and exit with the error's exit code.
