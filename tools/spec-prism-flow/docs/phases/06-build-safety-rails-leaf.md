...
## Scope
- spec_prism_flow/scope_guard.py
- spec_prism_flow/merge_gates.py
- spec_prism_flow/retry_budget.py
- spec_prism_flow/errors.py
- tests/test_scope_guard.py
- tests/test_merge_gates.py
...
### 7.1 `spec_prism_flow/scope_guard.py`
`matches_any(path: str, globs: list[str]) -> bool` — `fnmatch(path, glob)` over the list, `any()`. `check(changed_paths: list[str], scope_globs: list[str]) -> None` — raises `ScopeViolation(offending, scope_globs)` where `offending = [p for p in changed_paths if not matches_any(p, scope_globs)]`; returns `None` if `offending` is empty. Takes an already-computed `changed_paths` list (the diff-vs-base-ref computation is a git-ops concern, Phase 07) rather than a repo path — keeps this module free of subprocess calls. **Must use `fnmatch`, never `Path.match`** — the latter anchors from the right unless the pattern is absolute, so `Path("evil/src/App.tsx").match("src/*")` is `True`, a real bypass for a check whose entire job is exclusion. No built-in exceptions for any file type — scope is exactly the phase file's declared `Scope` list (Phase 01's `PhaseFile.scope`), nothing implicit.

### 7.2 `spec_prism_flow/merge_gates.py`
...
### 7.3 `spec_prism_flow/retry_budget.py`
...
- Run `uv run pytest tests/test_scope_guard.py tests/test_merge_gates.py tests/test_retry_budget.py tests/test_errors.py -v` and confirm all cases above pass, including the `fnmatch`-vs-`Path.match` bypass regression test.
