...
Reads every `docs/phases/*-leaf.md` file plus `graph.json`, plus the approved `requirements.md` and `OPEN_QUESTIONS.md`. Runs, in order, and reports all four independently (a failure on one does not skip the others):
