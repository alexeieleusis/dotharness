...
- `plan draft-overview` invokes `run_handoff` with `stage_name="draft_overview"` and, after it returns, raises if `00-overview.md` was not written; does not raise if `OPEN_QUESTIONS.md` is absent.
...
- `plan draft-requirements` invokes `run_handoff` with `stage_name="draft_requirements"` and raises if `requirements.md` was not written after it returns.
